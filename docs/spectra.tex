...
\begin{align*}
    \sop{H} = B\pmx{J(J - 1) & 0                  & 0     \\ 0 & (J + 1)(J + 2) & 0 \\ 0 & 0 & J(J + 1)} & + \lambda\pmx{\tfrac{-2J}{2J + 1} + \tfrac{2}{3} & \tfrac{2\sqrt{J(J + 1)}}{2J + 1} & 0 \\ \tfrac{2\sqrt{J(J + 1)}}{2J + 1} & \tfrac{-2(J + 1)}{2J + 1} + \tfrac{2}{3} & 0 \\ 0 & 0 & \tfrac{2}{3}} \\
                             & + \gamma\pmx{J - 1 & 0 & 0 \\ 0 & -(J + 2) & 0 \\ 0 & 0 & 1}
\end{align*}
...
The total molecular partition function can be expressed as the product of the individual electronic, vibrational, rotational, and nuclear components for symmetric molecules Hanson Eq. (5.4):
\begin{equation*}
    Q = Q_eQ_vQ_rQ_n
\end{equation*}
...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}
        \draw[thick] (0,0) -- (4,0) node[midway,below] {$E_1$};
        \draw[thick] (0,3) -- (4,3) node[midway,above] {$E_2$};

        \draw[->,ultra thick] (2,0) -- (2,3) node[midway,right] {$B_{12}$};

        \draw[->,ultra thick,decorate,decoration={snake,amplitude=2mm,segment length=8mm}] (3,3) -- (3,0)
        node[midway,right] {$A_{21}$};

        \draw[->,ultra thick,dashed] (1,3) -- (1,0) node[midway,left] {$B_{21}$};

        \draw[->,ultra thick] (3.5,3) .. controls (4.5,2) .. (3.5,0)
        node[midway,right] {Quenching (Non-radiative)};
    \end{tikzpicture}
\end{figure}
