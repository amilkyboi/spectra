...
class LifSimulation:
    def __init__(
        ...
        self.vib_bands: list[LifBand] = [
            LifBand(vib_band[0], vib_band[1], self) for vib_band in band_list
        ]
...
class LifBand:
    ...
    def get_branch_idx(
        self, rot_qn_up: int, rot_qn_lo: int, branch_range: range, branch_main: str
    ) -> list:
        lines = []
    ...
    def get_band_origin(self) -> float:
        ...
        vib_energy = terms.vibrational_term(
    ...
    def intensities_lif(self, rot_qn_up: int, rot_qn_lo: int) -> np.ndarray:
        intensities_lif = np.array(
            [line.intensity() for line in self.get_lif_lines(rot_qn_up, rot_qn_lo)]
        )
...
class LifLine:
    ...
    def predissociation(self) -> float:
        return self.molecule.prediss[f"f{self.branch_idx_lo}"][
            self.molecule.prediss["rot_qn"] == self.rot_qn_up
        ].iloc[0]
    ...
    def wavenumber(self) -> float:
        return (
            ...
            + terms.rotational_term(
                self.sim.state_up, self.band.vib_qn_up, self.rot_qn_up, self.branch_idx_up
            )
            - terms.rotational_term(
                self.sim.state_lo, self.band.vib_qn_lo, self.rot_qn_lo, self.branch_idx_lo
            )
        )
    ...
    def rot_boltzmann_factor(self) -> float:
        ...
        return np.exp(
            -terms.rotational_term(state, vib_qn, rot_qn, branch_idx)
            * cn.PLANC
            * cn.LIGHT
            / (cn.BOLTZ * self.sim.temp)
        )
...
def plot_lif(sim: LifSimulation, rot_qn_up: int, rot_qn_lo: int, colors: list) -> None:
    ...
    for idx, vib_band in enumerate(sim.vib_bands):
        ...
        plt.stem(
            wavelengths_lif,
            vib_band.intensities_lif(rot_qn_up, rot_qn_lo),
            colors[idx],
            markerfmt="",
