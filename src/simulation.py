...
class Simulation:
    ...
    def __init__(
        ...
        self.vib_bands: list[Band] = [
            Band(vib_band[0], vib_band[1], self) for vib_band in band_list
        ]
    ...
    def all_conv_data(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        wavenumbers_conv = np.linspace(
            wavenumbers_line.min(), wavenumbers_line.max(), inp.GRANULARITY
        )
        intensities_conv = convolve.convolve_brod(
            self, lines, wavenumbers_line, intensities_line, wavenumbers_conv
        )
