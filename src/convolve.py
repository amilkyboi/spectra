...
def convolve_brod(
    sim: Simulation,
    lines: list[Line],
    wavenumbers_line: np.ndarray,
    intensities_line: np.ndarray,
    ...
    for idx, (wave, intn) in enumerate(zip(wavenumbers_line, intensities_line)):
        intensities_conv += intn * broadening_fn(
            sim, lines, wavenumbers_conv, wave, idx, natural, collide
        )
...
def broadening_fn(
    sim: Simulation,
    lines: list[Line],
    convolved_wavenumbers: np.ndarray,
    wavenumber_peak: float,
    line_idx: int,
    natural: float,
    collide: float,
) -> np.ndarray:
    ...
    doppler: float = wavenumber_peak * np.sqrt(
        cn.BOLTZ * sim.temp / (sim.molecule.molecular_mass * (cn.LIGHT / 1e2) ** 2)
    )
...
def broadening_params(sim: Simulation) -> tuple[float, float]:
    ...
    collide: float = (
        (sim.pres * 10)
        * sim.state_lo.cross_section**2
        * np.sqrt(8 / (np.pi * sim.molecule.reduced_mass * cn.BOLTZ * sim.temp))
        / 2
    )
