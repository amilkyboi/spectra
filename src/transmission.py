...
total_partition = (
    np.exp(-vibrational_term(vib_qn_lo_list) * cn.PLANC * cn.LIGHT / (cn.BOLTZ * temperature))
).sum()
...
def number_density(vib_qn_lo):
    return (
        density
        * np.exp(-vibrational_term(vib_qn_lo) * cn.PLANC * cn.LIGHT / (cn.BOLTZ * temperature))
        / total_partition
    )
